...
const roomValidation = (roomRange, error) => {
  try {
    if (!isValidRange(roomRange)) error = "please input valid format";
...
export default class CheckError extends React.Component {
  render() {
    const roomRange = this.props.roomRange;
    const handicappedRoomrange = this.props.handicappedRoomrange;
    const error = this.props.error;
    return (
      <div>
        {roomValidation(roomRange, error)}
        {roomValidation(handicappedRoomrange, error)}
      </div>
    );
  }
}
